...
- `GAN.py`: GAN/VAE-GAN training (checkpoints + metrics logging)
- `LDM.py`: LDM training (AE stage + diffusion stage, checkpoints + metrics logging)
- `generate_run_artifacts.py`: post-training artifact generation from saved run files
...
Generate plots/images for all runs:

```bash
python generate_run_artifacts.py
```
...
`GAN.py` and `LDM.py` populate `hyperparameters.json`, `metrics.json`, and model checkpoints; plot/image files in `plots/` are generated by `generate_run_artifacts.py`.
...
## 8. Artifact Regeneration Utility

Regenerate run artifacts (plots/images) directly from saved checkpoints and metrics, without retraining:
...
- regenerates training curves from `metrics.json`
- regenerates latent-space and interpolation visuals from checkpoints
- regenerates diffusion decoded-intermediates strips for LDM runs
- regenerates deterministic 10-image reference-vs-reconstruction comparisons
...
- Class-based training organization in both pipelines (`Components`, `Training`) with explicit stage boundaries.
...
- Decoupled artifact generation workflow:
  - training scripts only log metrics/checkpoints
  - `generate_run_artifacts.py` regenerates all plots and visual outputs from saved runs
