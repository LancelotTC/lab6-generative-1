...
Out of curiosity, I also ran the current best-performing model on the `CXR` MedNIST subdataset (`LDM` run `20260323_120641_328800`), and the corresponding results are included below.
...
When it came to the results of the VAE-only GAN:

Interpolated gif showed visible hands, albeit blurry. Intermediate visualisations, as it went through the latent space, looked less convincing and more scary, but overall a better result that originally saw in class.

When it came to the results of the Latent Diffusion Model:

While the adversarial training curves look a bit odd in the beginning, they eventually stabilise to a result that is expected, and the visual result, be it in Diffusion Model - Latent Space (t-SNE 2D).png, MedNIST Interpolation.gif or Diffusion Model - Decoded Intermediates Every 100 Steps.png look beautiful, and what I would expect from a good model.
